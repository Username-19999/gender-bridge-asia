  ...
  const researchFrontiers = [
    {
      ...
      keyQuestions: t("Example questions: How do intersecting identities shape institutional power structures?", "示例问题：交叉身份如何塑造制度性权力结构？"),
    ...
    },
    {
      ...
      keyQuestions: t("Example questions: How do algorithms perpetuate gender bias in digital spaces?", "示例问题：算法如何在数字空间中延续性别偏见？"),
    ...
    },
    {
      ...
      keyQuestions: t("Example questions: How do labour policies reshape women's bargaining power?", "示例问题：劳动政策如何重塑女性的议价能力？"),
      color: "border-l-4 border-gray-400"
    },
    {
      ...
      keyQuestions: t("Example questions: What barriers prevent women from achieving political leadership?", "示例问题：哪些障碍阻止女性获得政治领导地位？"),
    ...
    },
    {
      ...
      keyQuestions: t("Example questions: How do legal systems recognize and protect non-binary identities?", "示例问题：法律体系如何承认和保护非二元身份？"),
      color: "border-l-4 border-[#00a19a]"
    },
    {
      ...
      keyQuestions: t("Example questions: How does climate change disproportionately affect women and marginalized groups?", "示例问题：气候变化如何不成比例地影响女性和边缘群体？"),
      color: "border-l-4 border-gray-400"
    },
    {
      ...
      keyQuestions: t("Example questions: How can alternative masculinities challenge patriarchal norms?", "示例问题：替代性男性气质如何挑战父权制规范？"),
      color: "border-l-4 border-gray-400"
    },
    {
      ...
      keyQuestions: t("Example questions: How does neoliberalism co-opt feminist movements?", "示例问题：新自由主义如何收编女权运动？"),
      color: "border-l-4 border-[#d62e39]"
    }
  ];
  ...
  const asiaResearch = [
    {
      ...
      color: "bg-gradient-to-br from-[#d62e39]/10 to-white"
    },
    {
      title: t("Taiwan", "台湾"),
      ...
      color: "bg-gradient-to-br from-[#00a19a]/10 to-white"
    },
    {
      ...
      color: "bg-gradient-to-br from-gray-400/10 to-white"
    },
    {
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-16">
              {researchFrontiers.map((item, index) => (
                <Card key={index} className={`p-6 hover:shadow-lg transition-shadow flex flex-col ${item.color}`}>
                  <div className="flex-1 flex flex-col">
                    <h3 className="text-lg sm:text-2xl font-bold mb-3 h-[3.5rem] line-clamp-2">{item.title}</h3>
                    <p className="text-sm sm:text-base text-gray-700 leading-relaxed mb-4 flex-1">{item.description}</p>
                    <p className="text-sm sm:text-base font-semibold text-gray-900 mb-4 italic border-l-2 border-gray-400 pl-3">
                      {item.keyQuestions}
                    </p>
                  </div>
                  <div className="mt-auto pt-4 border-t border-gray-200">
                    <span 
                      ...
                      className="inline-flex items-center text-sm sm:text-base text-gray-600 hover:text-gray-900 transition-colors cursor-pointer font-semibold"
                    >
                      {t("View related articles", "查看相关文章")} →
                    </span>
                  </div>
                </Card>
              ))}
            ...
            <div className="grid grid-cols-1 gap-6 mb-16">
              {asiaResearch.map((item, index) => (
                <Card key={index} className={`p-8 ${item.color} border-2 border-gray-200 hover:shadow-xl transition-shadow flex flex-col`}>
                  ...
                  <div className="mt-6">
                    <span 
                      onClick={handleViewArticles}
                      className="inline-flex items-center text-base sm:text-xl font-bold text-gray-600 hover:text-gray-900 transition-colors cursor-pointer"
                    >
                      {t("View related articles", "查看相关文章")} →
                    </span>
                  </div>
