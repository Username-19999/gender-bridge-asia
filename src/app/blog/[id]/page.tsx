      ...
      content: {
        en: `
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            <li><strong>Case Example:</strong> For the activity "cooking," a trained Conditional Random Field (CRF) model amplified the training set disparity (33% male agent presence) to 68% disparity at test time. The proportion of men in the agent role for "cooking" images dropped from approximately 33% in the training set to 16% in the predicted set.</li>
            <li><strong>Quantification:</strong> Trained models amplified existing bias by 5.0% for vSRL, and 3.6% in MLC on average.</li>
          </ul>
          <p class="mb-6">The authors emphasize that deploying such uncalibrated systems could not only reinforce existing social bias but also actually make them worse.</p>
          ...
          <p class="mb-6"><strong>Intervention Mechanism:</strong> RBA injects corpus-level constraints to ensure that model predictions follow the distribution observed in the training data. For instance, constraints ensure that the noun "man" occurs in the agent role of "cooking" as often as it did in the original training corpus.</p>
        ...
        zh: `
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            ...
            <li><strong>量化：</strong> 训练后的模型在 vSRL 任务中平均将现有偏见放大了 5.0%，在 MLC 任务中放大了 3.6%。</li>
          </ul>
          ...
          <p class="mb-6"><strong>干预机制：</strong> RBA 引入了语料库级别约束，以确保模型预测（例如，名词“man”在“烹饪”活动中作为代理人出现的频率）遵循原始训练语料库中观察到的分布。</p>
    ...
    "2": {
      ...
      content: {
        en: `
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            <li><strong>Exclusion of Consumer Behavior:</strong> Women's click-through rates (CTR) (0.167%) were actually higher than men's (0.131%), which invalidates the hypothesis that the algorithm reduced exposure because women were less likely to engage.</li>
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            ...
            <li><strong>Cost-Driven Disparate Impact:</strong> Consequently, female "eyeballs" are more expensive to advertise to. An algorithm optimizing solely for cost-effectiveness will favor the cheaper audience (men) when displaying a gender-neutral ad, leading to an apparent disparate impact due to crowding out.</li>
          </ul>
        ...
        zh: `
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            <li><strong>排除用户行为偏见：</strong> 女性的点击率（CTR）（0.167%）实际上高于男性（0.131%），这排除了算法因女性不点击而减少曝光的可能性。</li>
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            ...
            <li><strong>成本驱动的歧视影响：</strong> 因此，向女性投放广告的成本更高。一个仅仅优化成本效益的算法，在投放中立广告时，会倾向于选择成本更低的受众（男性），从而导致歧视影响。</li>
          </ul>
          ...
          <ul class="list-disc pl-6 mb-6 space-y-2">
            ...
            <li><strong>法律的紧张关系：</strong> 在美国，就业歧视法区分了"差别待遇"（故意歧视）和"歧视影响"（中立做法导致的负面影响）。但法律对于如何将定向广告纳入就业歧视框架尚未明确定论。</li>
